...
class Movimientos
{
    ...
    public function createAcuerdo(): void
    {
        ...
        $query = "INSERT INTO $this->tableName SET idMaquinaria=?, idRuta=?, kilometraje=?, tarifa=?, datecreated=?, idUsuario=?, nit=? ;";
        ...
        $stmt->bindParam(3, $this->kilometraje);
        $stmt->bindParam(4, $this->tarifa);
    ...
    public function readAllDaTableMovimiento(): void
    {
        ...
        $stmt = $this->conn->prepare("SELECT mo.id, tm.tipo, m.placa, (c.titulo)contrato, concat(r.origen, ' - ', r.destino)ruta , mo.kilometraje, mo.tarifa, u.nombres, mo.status 
                                      ...
                                      LEFT JOIN " . $this->tableRutaContrato . " rc ON r.id = rc.idRuta 
                                      LEFT JOIN " . $this->tableContratos . " c ON rc.idContrato = c.id 
                                      JOIN " . $this->tableUsuarios . " u ON mo.idUsuario = u.id WHERE 1 " . $searchQuery . " AND mo.status in(1, 2) AND m.status = 1 AND mo.nit =  " . $_SESSION['nit'] . " GROUP BY mo.id ORDER BY " . $columnName . " " . $columnSortOrder . " LIMIT :limit,:offset ");
        // --Bind values--
