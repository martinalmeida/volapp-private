...
class Alquiler
{
    ...
    public function readAllDaTableContrato(): void
    {
        ...
        $stmt = $this->conn->prepare("SELECT COUNT(*) AS allcount FROM " . $this->tableName . " a 
                                      ...
                                      JOIN " . $this->tableContratos . " v ON a.idContrato = c.id 
                                      JOIN " . $this->tableRutas . " r ON a.idRuta = r.id 
        ...
        $stmt = $this->conn->prepare("SELECT a.id, tm.tipo, m.placa, (c.titulo)contrato, concat(r.origen, ' - ', r.destino)ruta , a.standby, a.horaTarifa, u.nombres, a.status 
                                      ...
                                      JOIN " . $this->tableContratos . " v ON a.idContrato = c.id 
                                      JOIN " . $this->tableRutas . " r ON a.idRuta = r.id 
                                      JOIN " . $this->tableUsuarios . " u ON a.idUsuario = u.id WHERE 1 " . $searchQuery . " AND status in(1, 2) ORDER BY " . $columnName . " " . $columnSortOrder . " LIMIT :limit,:offset ");
        // --Bind values--
