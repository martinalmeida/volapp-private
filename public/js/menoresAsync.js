...
$(document).ready(function () {
  /* ---------  START Serverside Tabla ( tablaMenores ) ----------- */
  tablaMenores = $("#tablaMenores").DataTable({
    processing: true,
    orderClasses: true,
    deferRender: true,
    serverSide: true,
    ...
    ajax: {
      type: "POST",
      url: urlBase + "routes/menores/readAllDaTable",
    },
    dom:
...
function readPermisos() {
  $.ajax({
    dataType: "json",
    url: urlBase + "routes/menores/read",
    type: "GET",
    beforeSend: function () {},
...
function writePermisos() {
  $.ajax({
    dataType: "json",
    url: urlBase + "routes/menores/write",
    type: "GET",
    beforeSend: function () {},
    success: function (result) {
...
function registrar(form) {
  ...
  if (respuestavalidacion) {
    ...
    if (edit == true) {
      peticion = urlBase + "routes/menores/update";
    } else if (edit == false) {
      $("#archivoBase64").html("");
      peticion = urlBase + "routes/menores/create";
    } else if (edit == null) {
      return false;
    }
    $.ajax({
        ...
        switch (estado) {
          ...
          case "1":
            if (edit == false) {
              Swal.fire({
                icon: "success",
                title: "<strong>Equipo Creado</strong>",
                html: "<h5>El equipo se ha registrado exitosamente</h5>",
              ...
              Swal.fire({
                icon: "success",
                title: "<strong>Equipo Editado</strong>",
                html: "<h5>El Equipo se ha editado exitosamente</h5>",
                showCloseButton: false,
                confirmButtonText: "Aceptar",
                confirmButtonColor: "#64a19d",
                backdrop: true,
          ...
          case "5":
            html +=
              ...
              '<i class="fal fa-exclamation-triangle"></i></div>' +
              '<div class="flex-1 text-warning"><span class="h5 m-0 fw-700">Adjunte el Archivo de Documentación del Equipo </span></div>' +
...
function editarRegistro(id) {
  ...
  edit = true;
  $.ajax({
    data: { idMenor: id }, //datos a enviar a la url
    dataType: "json", //Si no se especifica jQuery automaticamente encontrará el tipo basado en el header del archivo llamado (pero toma mas tiempo en cargar, asi que especificalo)
    url: urlBase + "routes/menores/getData", //url a donde hacemos la peticion
    type: "POST",
    beforeSend: function () {
      // $(".overlayCargue").fadeIn("slow");
    },
      ...
      switch (estado) {
        ...
        case "1":
          ...
          html +=
            ...
            '<input type="hidden" id="idMenor" name="idMenor" value="' +
            result.data.id +
            '">' +
          ...
          $("#btnRegistro").text("Editar Equipo");
...
function statusRegistro(id, status) {
  $.ajax({
    ...
    dataType: "json", //Si no se especifica jQuery automaticamente encontrará el tipo basado en el header del archivo llamado (pero toma mas tiempo en cargar, asi que especificalo)
    url: urlBase + "routes/menores/status", //url a donde hacemos la peticion
    type: "POST",
    beforeSend: function () {
      ...
      switch (estado) {
        ...
        case "1":
          Command: toastr["success"](
            "Estado del equipo cambiado exitosamente.",
            "Estado Cambiado"
          );
...
function eliminarRegistro(id) {
  Swal.fire({
    ...
    text: "Se eliminara el Equipo del sistema!",
    type: "warning",
    showCancelButton: true,
    ...
    preConfirm: function () {
      $.ajax({
        data: { idMenor: id },
        dataType: "json", //Si no se especifica jQuery automaticamente encontrará el tipo basado en el header del archivo llamado (pero toma mas tiempo en cargar, asi que especificalo)
        url: urlBase + "routes/menores/delete", //url a donde hacemos la peticion
        type: "POST",
        beforeSend: function () {
          // $("#overlayText").text("Cerrando Sesión...");
          // $(".overlayCargue").fadeOut("slow");
        ...
        success: function (result) {
          ...
          switch (estado) {
            ...
            case "1":
              Command: toastr["success"](
                "El equipo se ha eliminado satisfactoriamente.",
                "Equipo Eliminado"
              );
...
function showModalRegistro() {
  reset();
  $("#alertaForm").html("");
  $("#btnRegistro").text("Registrar Equipo");
  $("#btnRegistro").attr("onclick", "registrar('frmRegistro');");
  $("#ModalRegistro").modal({
    backdrop: "static",
    keyboard: false,
...
function reajustDatatables() {
  tablaMenores.columns.adjust().draw();
}
