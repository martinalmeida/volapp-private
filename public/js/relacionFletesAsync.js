...
function writePermisos() {
  $.ajax({
    dataType: "json",
    url: urlBase + "routes/informesRelacion/write",
    type: "GET",
    beforeSend: function () {},
    success: function (result) {
...
function formRelacion() {
  ...
  html +=
    ...
    '<button type="button" id="btnGenerar" class="btn btn-info btn-pills btn-block waves-effect waves-themed">Generar la Relación por Fletes para Cobrar <i class="fal fa-sack-dollar"></i></button>';
...
function generaRelacion() {
  ...
  tablaInforme = $("#tablaInforme").DataTable({
    ...
    destroy: true,
    ajax: {
      url: urlBase + "routes/informesRelacion/relacionFlete",
      type: "POST",
      data: {
        placa: placa,
        contrato: contrato,
    ...
    buttons: [
      {
        ...
        messageTop: "Relación a cobrar de Fletes de maquinarias",
        title: "Relación por Fletes",
      },
    ],
    columns: [
      { data: "id" },
