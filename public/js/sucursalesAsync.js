...
$(document).ready(function () {
  /* ---------  START Serverside Tabla ( tablaSucursales ) ----------- */
...
function readPermisos() {
  $.ajax({
    dataType: "json",
    url: urlBase + "routes/sucursales/read",
    type: "GET",
    beforeSend: function () {},
...
function writePermisos() {
  $.ajax({
    dataType: "json",
    url: urlBase + "routes/sucursales/write",
    type: "GET",
    beforeSend: function () {},
    success: function (result) {
...
function registrar(form) {
  ...
  if (respuestavalidacion) {
    ...
    if (edit == true) {
      peticion = urlBase + "routes/sucursales/update";
    } else {
      peticion = urlBase + "routes/sucursales/create";
    }
    $.ajax({
        ...
        switch (estado) {
          ...
          case "1":
            if (edit == false) {
              Swal.fire({
                icon: "success",
                title: "<strong>Sucursal Creada</strong>",
                html: "<h5>La sucursal se ha registrado exitosamente</h5>",
              ...
              Swal.fire({
                icon: "success",
                title: "<strong>Sucursal Editada</strong>",
                html: "<h5>La Sucursal se ha editado exitosamente</h5>",
                showCloseButton: false,
                confirmButtonText: "Aceptar",
                confirmButtonColor: "#64a19d",
                backdrop: true,
...
function editarRegistro(id) {
  ...
  edit = true;
  $.ajax({
    data: { idSucursal: id }, //datos a enviar a la url
    dataType: "json", //Si no se especifica jQuery automaticamente encontrará el tipo basado en el header del archivo llamado (pero toma mas tiempo en cargar, asi que especificalo)
    url: urlBase + "routes/sucursales/getData", //url a donde hacemos la peticion
    type: "POST",
    beforeSend: function () {
      // $(".overlayCargue").fadeIn("slow");
    },
    success: function (result) {
      ...
      switch (estado) {
        ...
        case "1":
          $("#btnRegistro").text("Editar Sucursal");
...
function statusRegistro(id, status) {
  $.ajax({
    ...
    dataType: "json", //Si no se especifica jQuery automaticamente encontrará el tipo basado en el header del archivo llamado (pero toma mas tiempo en cargar, asi que especificalo)
    url: urlBase + "routes/sucursales/status", //url a donde hacemos la peticion
    type: "POST",
    beforeSend: function () {
      ...
      switch (estado) {
        ...
        case "1":
          Command: toastr["success"](
            "Estado de la sucursal cambiado exitosamente.",
            "Estado Cambiado"
          );
  ...
  Swal.fire({
    ...
    preConfirm: function () {
      $.ajax({
        data: { idSucursal: id },
        dataType: "json", //Si no se especifica jQuery automaticamente encontrará el tipo basado en el header del archivo llamado (pero toma mas tiempo en cargar, asi que especificalo)
        url: urlBase + "routes/sucursales/delete", //url a donde hacemos la peticion
        type: "POST",
        beforeSend: function () {
          // $("#overlayText").text("Cerrando Sesión...");
          // $(".overlayCargue").fadeOut("slow");
        ...
        success: function (result) {
          ...
          switch (estado) {
            ...
            case "1":
              Command: toastr["success"](
                "La sucursal se ha eliminado satisfactoriamente.",
                "Sucursal Eliminada"
              );
...
function showModalRegistro() {
  reset();
  $("#btnRegistro").text("Registrar Sucursal");
  $("#btnRegistro").attr("onclick", "registrar('frmRegistro');");
  $("#ModalRegistro").modal({
    backdrop: "static",
    keyboard: false,
...
function reajustDatatables() {
  tablaSucursales.columns.adjust().draw();
}
